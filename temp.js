...
setTimeout(() => {
    if (typeof renderVennDiagram !== 'undefined') {
        renderVennDiagram(generateMockVennData());
        document.getElementById('playground-venn').style.display = 'block';
    }
}, 3000);
...
function fetchSearches() {
  fetch('/api/searches').then(r => r.json()).then(data => {
    ...
    c.innerHTML = '';
    data.forEach(s => {
      // Create search item with progress bar
      // Il calcolo in comune non lo abbiamo al momento, mettiamo una dummy bar
      let div = document.createElement('div');
      div.style.cssText = 'padding:10px; background:#f8f9fa; border:1px solid #ddd; border-radius:8px; cursor:pointer;';
      div.innerHTML = `
        <div style="font-weight:bold; font-size:0.8rem; color:#333;">${s.query}</div>
        <div style="font-size:0.65rem; color:#777; margin-top:2px;">${s.file}</div>
        <div style="width:100%; height:4px; background:#eee; margin-top:6px; border-radius:2px; overflow:hidden; display:flex;">
          <div style="width:40%; background:var(--orange); height:100%;"></div>
          <div style="width:20%; background:#2ecc71; height:100%;"></div>
        </div>
      `;
      // Clicking a search should probably reload the app for that file
      div.onclick = () => { alert("Integrazione da fare: ricaricare la pagina puntando a " + s.file); };
      c.appendChild(div);
    });
